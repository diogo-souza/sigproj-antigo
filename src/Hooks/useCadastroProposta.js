...
const useCadastroProposta = () => {
  ...
  const addProjeto = (uuid, titulo) => {
    let { projetosVinculados } = values;
    projetosVinculados.push({ uuid, titulo });
    const id = 'projetosVinculados';
    setValues({
      ...values,
      [id]: projetosVinculados,
    });
  };
  ...
  const removeProjeto = (index) => {
    let { projetosVinculados } = values;
    projetosVinculados.splice(index, 1);
    setValues({
      ...values,
      projetosVinculados,
    });
...
export default useCadastroProposta;
